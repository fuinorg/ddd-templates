	...
	override toString() {
		if (makeAbstract) {
			'''	
				...
				«modifiers» abstract void «method.name»(«new SrcParamsDecl(ctx, method.allVariables)»)«new SrcThrowsExceptions(ctx, method.allExceptions)»;
			'''
		} else {
			'''	
				«new SrcMethodJavaDoc(ctx, method)»
				«modifiers» void «method.name»(«new SrcParamsDecl(ctx, method.allVariables)»)«new SrcThrowsExceptions(ctx,	method.allExceptions)» {
					// TODO Implement!
				}
			'''
		}
